...
```{r}
test.wmax <- cycling.data %>% 
  select(id, period, timepoint, test, watt) %>% 
  filter(test == "max", (timepoint == 2 | timepoint == 3), period == 1) %>% 
  select(id, timepoint, watt) %>% 
  pivot_wider(names_from = timepoint, values_from = watt, names_prefix = "timepoint_", values_fn = list(watt = mean)) %>% 
  mutate(diff = timepoint_3 - timepoint_2) %>% 
  summarise (mean_diff = mean(diff, na.rm = TRUE),
             mean = mean(c(timepoint_2, timepoint_3),na.rm = T),
             sd_diff = sd(diff, na.rm = TRUE),
...
```{r}
test.w15 <- cycling.data %>% 
  select(id, period, timepoint, test, watt) %>% 
  filter(test == "per", (timepoint == 2 | timepoint == 3), period == 1) %>% 
  select(id, timepoint, watt) %>% 
  pivot_wider(names_from = timepoint, values_from = watt, names_prefix = "timepoint_", values_fn = list(watt = mean)) %>% 
  mutate(diff = timepoint_3 - timepoint_2) %>% 
  summarise (mean_diff = mean(diff, na.rm = TRUE),
...
```{r}
icc.test <- lmm_wmax %>%
  select(change, id, period) %>% 
  pivot_wider(names_from = period, values_from = change, names_prefix = "period_") %>% 
  print()

matrix.icc <- as.matrix(icc.test[, c("period_1", "period_2")])

ICC <- ICC(matrix.icc, alpha=.05)
print(ICC)
```
