...
###combined results thats not in figures
```{r}
print(icc_df.vo2)
print(icc_df.wmax)
print(icc_df.w15)
...
lmm_bv <- hb.data %>%
  print()
  ungroup() %>% 
  select(timepoint, period, id, group, bv) %>% 
  filter(!id %in% c(6,11, 20, 21, 25, 35, 37, 52, 53, 54, 56, 57),
         !period %in% (3),
         !group %in% ("control")) %>%
  drop_na() %>% 
  pivot_wider(names_from = timepoint, values_from = bv) %>% 
  mutate(change = post - pre) %>% 
  print()
   
  model.bv <- lmer(change ~ period + (1 | id), data = lmm_bv)
  summary(model.bv)
  ci.lmm.bv <- confint(model.bv)
  l.ci.lmm.bv <- ci.lmm.bv[4, 1]
  u.ci.lmm.bv <- ci.lmm.bv[4, 2]
...
irr.icc.bv.result <- icc(irr.icc.bv, model = "twoway", type = "agreement", unit = "single")

# View the ICC result
print(irr.icc.bv.result)

# Extract ICC and confidence intervals
icc_value.bv <- irr.icc.bv.result$value
lower_ci.bv <- irr.icc.bv.result$lbound
upper_ci.bv <- irr.icc.bv.result$ubound

# Create a dataframe with renamed columns
icc_df.bv <- data.frame(
  icc.bv = paste0(
    round(icc_value.bv, 2), 
    " [", round(lower_ci.bv, 2), ", ", round(upper_ci.bv, 2), "]"))
